...
start_docker_stack() {
  ...
  if ! docker_runtime_healthy; then
    log "⚠️ Attempting Docker daemon restart before compose startup..."
    sudo systemctl restart docker || true
    sleep 3
    if ! docker_runtime_healthy; then
      log "❌ Docker runtime still unhealthy after daemon restart."
      log "Hint: reboot host and upgrade Docker/containerd packages."
      rm -f "${compose_log}"
      return 1
    fi
  fi
