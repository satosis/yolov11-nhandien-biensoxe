...
- **Lỗi Docker shim segfault (`unexpected fault address`, `failed to start shim`)**: `install.sh` sẽ tự restart Docker daemon và retry 1 lần. Nếu vẫn lỗi, nâng cấp Docker/containerd rồi chạy lại (`sudo apt-get install --only-upgrade docker-ce docker-ce-cli containerd.io`).
